 ...
 * @Author: RongWei
 * @Date: 2020-01-23 16:24:58
 * @LastEditors: RongWei
 * @LastEditTime: 2020-01-23 16:24:59
 */
/*
...
var simplifyPath = function (path) {
    var orgPath = path.split(/\/+/);
    var newPath = [];
    for (var i = 0; i < orgPath.length; i += 1) {
        var character = orgPath[i];
        if (character === '..') {
            newPath.pop();
        } else if (character && character !== '.') {
            newPath.push(character);
        }
    }
    return '/' + newPath.join('/');
};
